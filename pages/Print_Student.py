...
dash.register_page(__name__, path='/Print_Student')
...
if not isinstance(df_ms, pd.DataFrame):
    ...
    layout = dbc.Container([color_mode_switch,dbc.Row([
            dbc.Col([
                ...
                dbc.Table.from_dataframe(df_ms[df_ms['time_p']==1].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
                html.Div(children="day 1 - Afternoon"),
                dbc.Table.from_dataframe(df_ms[df_ms['time_p']==2].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
                html.Div(children="day 2 - Morning"),
                dbc.Table.from_dataframe(df_ms[df_ms['time_p']==3].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
            ]),dbc.Col([
                html.Div(children="Elementary School"),
                html.Div(children="day 1 - Morning"),
                dbc.Table.from_dataframe(df_ls[df_ls['time_p']==1].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
                html.Div(children="day 1 - Afternoon"),
                dbc.Table.from_dataframe(df_ls[df_ls['time_p']==2].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
                html.Div(children="day 2 - Morning"),
                dbc.Table.from_dataframe(df_ls[df_ls['time_p']==3].drop(columns=['time_p','Time']), striped=True, bordered=True, hover=True),
            ])])
    ],fluid=True)
